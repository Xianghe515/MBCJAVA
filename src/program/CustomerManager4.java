...
public class CustomerManager4 {
	 ...
	 * 고려사항 
	 * 1. 객체 -
	 * Customer 객체가 직렬화되어있는가 
	 * 2. 데이터를 저장하고 불러오는 시점 
	 * 	- 데이터를 저장하는 메소드(writeCustomerData) -> 데이터 입력・수정 시, 프로그램 종료 시 
	 * 	- 데이터를 불러오는 메소드(readCustomerData)  -> 프로그램 시작 전
	 */
	...
	public static void insertCustomerData() {
		...
		/*
		 * Customer c1 = new Customer(); c1.setName(name); c1.setGener(gender);
		 * c1.setEmail(email); c1.setBirthYear(birthYear);
		 * 
		 */
		cuslist.add(c1);
	}
...
//	            cus.setName(scan.next());
	...
	public static void writeCustomerData() {
		...
		File file = new File("D:/Temp/CustomerManager.sav");
		...
		} catch (IOException e) {	// 입출력에 대한 예외
			System.out.println(e.getMessage());
		} finally {		// 객체 정리할 때 많이 사용(객체가 정리 되어있는데 혹여나 남아있을 경우를 대비)
			try {
				if (oos != null) oos.close();
				if (fos != null) fos.close();	// 객체 != null -> 객체가 남아있음 -> '객체가 남아있다면 정리해줘'
	...
	public static void readCustomerData() {	// 데이터 객체 접근 가능
		// 객체 직렬화를 이용한 읽기
		...
		try {
			...
			cuslist = (List<Customer>)ois.readObject();	// ClassNotFoundException -> catch 블록 하나 더 만들어서 해결
